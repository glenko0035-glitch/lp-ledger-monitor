...
async function discoverWalletTransactions(afterBlock){for(const direction of ['fromAddress','toAddress']){let pageKey;for(let page=0;page<20;page++){const filter={fromBlock:hex(afterBlock+1),toBlock:hex(head),[direction]:WALLET,category:['external','erc20','erc721','erc1155'],withMetadata:false,excludeZeroValue:false,maxCount:'0x3e8',...(pageKey?{pageKey}:{})};const data=await rpc('alchemy_getAssetTransfers',[filter]);for(const item of data.transfers||[])if(item.hash)found.add(item.hash);if(!data.pageKey)break;pageKey=data.pageKey;}}}
