...
const isFunding=funding.includes(hash);const recognized=isFunding||knownPosition; if(!recognized)pending.push(hash);
rows.push({pool:isFunding?0:knownPosition?3:-1,hash,time:'',action:isFunding?'外部本金到賬':knownPosition?(BigInt.asIntN(256,mw[2])===0n?'收取費用':BigInt.asIntN(256,mw[2])>0n?'加倉 / 費用抵扣':'減少流動性'):'新交易待分類',usd:net(r,USDG,6),token:net(r,PONS,18),symbol:'PONS',recognized});}
...
const gas3=rows.filter(r=>r.pool===3).reduce((a,r)=>a+r.gasUsd,0),gasAll=rows.reduce((a,r)=>a+r.gasUsd,0),capital=9801.946811812868;
...
const nativeResidual=monitor.nativeBalance===null?0:balance-monitor.nativeBalance+newGas;
...
const snapshot={asOf,block:head,reconciled,monitor,apr,aprSamples:samples.slice(-720),pool3:{...s,collected,walletPons:walletP,walletUsdg:walletU,capital,value,gas:gas3,pnl:value-capital-gas3,roi:(value-capital-gas3)/(capital+gas3)*100},totals:{gas:gasAll,frontCost:9.367608,external:11334.61,value:11925.7305870468-9129.70486823247+value,pnl:2606.6796077-1346.627199+(value-capital-gas3)},newOperations:two.map(h=>rows.find(r=>r.hash===h))};
