...
let lastRequest=0;
export async function rpc(method,params){for(let i=0;i<3;i++){let retryAfter=0;try{await new Promise(r=>setTimeout(r,Math.max(0,1200-(Date.now()-lastRequest))));lastRequest=Date.now();const res=await fetch(RPC,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method,params}),signal:AbortSignal.timeout(15000)});const h=res.headers.get('retry-after');retryAfter=h?(Number.isFinite(Number(h))?Number(h)*1000:Math.max(0,Date.parse(h)-Date.now())):0;if(!res.ok)throw Error('RPC HTTP '+res.status);const j=await res.json();if(j.error)throw Error(JSON.stringify(j.error));return j.result}catch(e){if(i===2||retryAfter>60000)throw e;await new Promise(r=>setTimeout(r,Math.max(retryAfter,/429/.test(String(e))?10000*2**i:2000*2**i)))}}}
